...
const Hero = () => {
  ...
  const rotateX = useSpring(useTransform(y, [-0.5, 0.5], [2.5, -2.5]), {
    stiffness: 100,
    damping: 20,
  });
  const rotateY = useSpring(useTransform(x, [-0.5, 0.5], [-2.5, 2.5]), {
    stiffness: 100,
    damping: 20,
  });
  const moveX = useSpring(useTransform(x, [-0.5, 0.5], [-10, 10]), {
    stiffness: 100,
    damping: 20,
  });
  const moveY = useSpring(useTransform(y, [-0.5, 0.5], [-10, 10]), {
    stiffness: 100,
    damping: 20,
  });
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-0 z-0 overflow-hidden">
              ...
              <motion.div
                ...
                className={
                  currentSlide === 0
                    ...
                    : slides[currentSlide].bgClassName ||
            ...
            {currentSlide === 0 &&
              (
                slides[currentSlide] as (typeof slides)[0] & {
                  tabletBg?: string;
                }
              ).tabletBg && (
                <AnimatePresence mode="wait">
        ...
        <div
          className="hidden xl:block absolute inset-0 w-full h-full pointer-events-none z-20 overflow-visible"
          style={{ perspective: "1000px" }}
        >
          <AnimatePresence mode="wait">
        ...
        <div className="absolute inset-0 z-30 w-full h-full p-8 md:p-16 flex flex-col justify-end items-start pointer-events-none pb-28 md:pb-24">
          <div className="pointer-events-auto">
            ...
            >
              <Link
                href="/best-sellers"
                className="group relative glass-card text-white px-6 py-3 md:px-10 md:py-4 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] transition-all shadow-2xl hover:scale-105 active:scale-95 hover:bg-white hover:text-black"
              >
        ...
        >
          <span className="text-[10px] text-white/40 uppercase tracking-[0.3em] font-bold mb-2">
            Scroll
          </span>
          <svg
